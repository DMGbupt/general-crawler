...
class ChinaevDao(Dao):
    ...
    def save(self, news):
        try:
            ...
            # 插入资讯表单
            cur.execute(ChinaevDao._sql_save_info, param_info)
            if news['atc'] == 1:
                cur.execute(ChinaevDao._sql_load_info_id.format("crawl_url"),(unicode( news['crawl_url']),) )
                info_id = cur.fetchone()[0]
                param_info_attachment = (news['atc_name'], news['atc_path'], info_id)
                cur.execute(ChinaevDao._sql_save_info_attachment, param_info_attachment)
            self.conn.commit()
        except Exception, e:
            spider_logger.error("[MySQL] Flush chinaev news to database failed, because of {0} !".format(e))
            self.conn.rollback()
        finally:
            cur.close()
